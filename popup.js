...
document.getElementById('increaseElement').onclick = () => {
  withActiveTab((tabId) => sendCommandToTab(tabId, 'enableElementResize', { dir: 'up' }));
};
document.getElementById('decreaseElement').onclick = () => {
  withActiveTab((tabId) => sendCommandToTab(tabId, 'enableElementResize', { dir: 'down' }));
};
document.getElementById('resetAll').onclick = () => {
  withActiveTab((tabId) => sendCommandToTab(tabId, 'resetAll'));
};

document.getElementById('highContrast').onclick = () => withActiveTab(tabId => sendCommandToTab(tabId, 'toggleHighContrast'));
document.getElementById('dyslexiaMode').onclick = () => withActiveTab(tabId => sendCommandToTab(tabId, 'toggleDyslexia'));
document.getElementById('largeCursor').onclick = () => withActiveTab(tabId => sendCommandToTab(tabId, 'toggleLargeCursor'));
document.getElementById('highlightLinks').onclick = () => withActiveTab(tabId => sendCommandToTab(tabId, 'toggleHighlightLinks'));

document.getElementById('focusMode').onclick = () => withActiveTab(tabId => sendCommandToTab(tabId, 'toggleFocusMode'));
...
document.getElementById('readText').onclick = async () => {
  ...
  try {
    ...
    if (res && res.text && res.text.length > 0) {
      ...
      chrome.tts.speak('Nenhum texto encontrado para leitura.', { rate, enqueue: true });
    }
  } catch (err) {
    console.error('Erro ao obter texto:', err);
    chrome.tts.speak('Erro ao obter o texto da página.', { rate, enqueue: true });
  }
};
...
function formatNumber(v){
  ...
  return Number.isInteger(v) ? v.toString() : v.toFixed(6).replace(/(?:\.0+|0+)$/,'');
}
